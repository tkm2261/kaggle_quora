...
leaks_input = Input(shape=(int(leaks.shape[1]),))
...
all_pred = np.zeros(labels.shape[0])
test_preds = []
...
for idx_train, idx_val in cv.split(data_1, labels):
    model.load_weights('param.hdf5')
    bst_model_path = STAMP + '_%s.h5' % cnt
    ...
    _preds = model.predict([data_1_val, data_2_val, leaks_val], batch_size=8192, verbose=1)
    _preds += model.predict([data_2_val, data_1_val, leaks_val], batch_size=8192, verbose=1)
    _preds /= 2
    #bst_val_score = min(hist.history['val_loss'])
    all_pred[idx_val] = _preds

    _preds = model.predict([test_data_1, test_data_2, test_leaks], batch_size=8192, verbose=1)
    _preds += model.predict([test_data_2, test_data_1, test_leaks], batch_size=8192, verbose=1)
    _preds /= 2
    test_preds.append(_preds)

with open('lstm_all_pred_0527.pkl', 'wb') as f:
...
with open('tmp.pkl', 'wb') as f:
    pickle.dump(test_preds, f, -1)

preds = np.mean(test_preds, axis=0)
with open('lstm_test_pred_0527.pkl', 'wb') as f:
