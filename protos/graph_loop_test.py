...
#cliques = sorted(list(nx.cycle_basis(G)), key=lambda x: (len(x), max(map(str, x))))
#cliques = sorted(list(nx.find_cycle(G)), key=lambda x: (len(x), max(map(str, x))))
cliques = sorted(list(nx.connected_components(G)), key=lambda x: (len(x), max(map(str, x))))
