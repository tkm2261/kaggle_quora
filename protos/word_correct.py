...
def split_into_words(text):
    return [get(t.lemma_)
            for t in nlp(text.lower())
            if t.lemma_ not in stops]
...
def load_data():
    ...
    with open('tfidf_mat_aspell.pkl', 'wb') as f:
...
def train():
    with open('count_mat_aspell.pkl', 'rb') as f:
        count_mat = pickle.load(f)
    logger.info('count_mat {}'.format(count_mat.shape))
    with open('tfidf_mat_aspell.pkl', 'rb') as f:
    ...
    df_train.to_csv('count_tfidf_train_clean2_aspell.csv', index=False)
    ...
    df_test.to_csv('count_tfidf_test_clean2_aspell.csv', index=False)
