...
def train():
    with open('count_mat_brown.pkl', 'rb') as f:
        count_mat = pickle.load(f)
    logger.info('count_mat {}'.format(count_mat.shape))
    with open('tfidf_mat_brown.pkl', 'rb') as f:
    ...
    df_train.to_csv('count_tfidf_train_clean2_brown.csv', index=False)
    ...
    df_test.to_csv('count_tfidf_test_clean2_brown.csv', index=False)
