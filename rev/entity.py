...
def make_idf():
    ...
    df2 = pandas.read_csv('../data/test_clean2_rev.csv',
                          usecols=['question1', 'question2']).values
    """
    p = Pool()
    _ret = p.map(aaa, df[:, 0])
...
def load_data(row):
    row[0] = PAT2.sub('\g<1>\g<2>', str(row[0]))
    row[1] = PAT2.sub('\g<1>\g<2>', str(row[1]))
    q1 = parser(str(row[0]))
    q2 = parser(str(row[1]))
    ...
    set_up1 = set([ele.lower() for ele in set_ent1 if PAT.search(ele) is not None])
    set_up2 = set([ele.lower() for ele in set_ent2 if PAT.search(ele) is not None])
...
if __name__ == '__main__':
    ...
    print(ret[:100])
    with open('train_ent.pkl', 'wb') as f:
        pickle.dump(ret, f, -1)
    ...
    ret = numpy.array(list(p.map(load_data, df)))
    with open('test_ent.pkl', 'wb') as f:
        pickle.dump(ret, f, -1)
