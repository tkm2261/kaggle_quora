...
def train():
    ...
    with open('tfidf_norm_mat.pkl', 'rb') as f:
        tfidf_mat = pickle.load(f)
    logger.info('tfidf_mat {}'.format(tfidf_mat.shape))
    map_train, map_test, train_num = make_idmap()

    df = pandas.read_csv('../data/train_clean2_rev.csv')[['question1', 'question2']].fillna('').values
    df_train = _train(count_mat[:train_num], tfidf_mat[:train_num], df, map_train)
    df_train.to_csv('count_tfidf_norm_train_clean2_rev.csv', index=False)

    df = pandas.read_csv('../data/test_clean2_rev.csv')[['question1', 'question2']].fillna('').values
    df_test = _train(count_mat[train_num:], tfidf_mat[train_num:], df, map_test)
    df_test.to_csv('count_tfidf_norm_test_clean2_rev.csv', index=False)
...
def _train(count_mat, tfidf_mat, df, map_train):
    ...
    data = numpy.concatenate([wt1, wt2, wt1_rate, wt2_rate, max_tfidf, sum_tfidf, tfidf_rate, same_w1, same_w2, tfidf_rate_dup,
                              wi1, wi2, wi1_rate, wi2_rate, max_idf, sum_idf, idf_rate, same_wi1, same_wi2, idf_rate_dup
                              ], axis=1)
    return pandas.DataFrame(data, columns=['wt1', 'wt2', 'wt1', 'wt2', 'max_tfidf',  'sum_tfidf', 'rate_tfidf', 'same_w1', 'same_w2', 'tfidf_rate_dup',
