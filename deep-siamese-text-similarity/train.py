...
df_train = pd.read_csv('../data/train.csv')[['question1', 'question2', 'is_duplicate']].values


max_document_length = 30
inpH = InputHelper()
train_set, dev_set, vocab_processor, sum_no_of_batches, sample_weight = inpH.getDataSets(
    df_train, max_document_length, 10, FLAGS.batch_size)

# Training
# ==================================================
logger.info("starting graph def")
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        logger.info("initialized siameseModel object")

    grads_and_vars = optimizer.compute_gradients(siameseModel.loss)
    tr_op_set = optimizer.apply_gradients(grads_and_vars, global_step=global_step)
    logger.info("defined training_ops")
    ...
    logger.info("init all variables")
    ...
    def train_step(x1_batch, x2_batch, y_batch):
        ...
        logger.info("TRAIN {}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
        #logger.info(y_batch, dist, d)
    ...
    def dev_step(x1_batch, x2_batch, y_batch):
        ...
        d[d > 1.0] = 0
        accuracy = np.mean(y_batch == d)
        logger.info("DEV {}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
        #logger.info(y_batch, dist, d)
        return d
    ...
    for nn in range(sum_no_of_batches * FLAGS.num_epochs):
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            for db in dev_batches:
                ...
                if len(y_dev_b) < 1:
                    continue
                pred = dev_step(x1_dev_b, x2_dev_b, y_dev_b)
                preds = np.r_[preds, pred]

            score = log_loss(dev_set[2], preds, sample_weight=sample_weight[1])
            score2 = - roc_auc_score(dev_set[2], preds, sample_weight=sample_weight[1])
            logger.info('loss: {}'.format(score))
            logger.info('auc: {}'.format(score2))

        if current_step % FLAGS.checkpoint_every == 0:
            saver.save(sess, checkpoint_prefix, global_step=current_step)
            tf.train.write_graph(sess.graph.as_graph_def(), checkpoint_prefix,
                                 "graph" + str(nn) + ".pb", as_text=False)
            logger.info("Saved model {} with sum_accuracy={} checkpoint to {}\n".format(
